...
export default function Timeline({ fabricCanvas }: TimelineProps) {
    const {
        ...
        animatedObjects,
        setCurrentTime, setDuration, setLoopMode, setTimelineZoom,
    ...
    useEffect(() => {
        compileTimeline(animatedObjects, fabricCanvas, loopMode as LoopMode);
        if (isPlaying) {
            timelineController.play();
        }
    }, [loopMode, animatedObjects, fabricCanvas]);

    // ⚡ Direct DOM playhead update — no React re-render during 60fps playback
    useEffect(() => {
        timelineController.onPlayhead((time: number) => {
            const dur = durationRef.current;
            const pct = dur > 0 ? Math.min(100, Math.max(0, (time / dur) * 100)) : 0;
            const labelText = `${time.toFixed(2)}s`;
            if (rulerPlayheadRef.current) {
                rulerPlayheadRef.current.style.left = `${pct}%`;
            }
            const trackLeft = `calc(${LABEL_WIDTH}px + (100% - ${LABEL_WIDTH}px) * ${pct / 100})`;
            if (trackPlayheadRef.current) {
                trackPlayheadRef.current.style.left = trackLeft;
            }
            if (dragHandleRef.current) {
                dragHandleRef.current.style.left = trackLeft;
            }
            if (timeLabelRef.current) {
                timeLabelRef.current.textContent = labelText;
            }
        });
        return () => timelineController.onPlayhead(null);
    }, []);
    ...
    const getCurrentPropertyValue = useCallback((layerId: string, property: AnimatableProperty): any => {
        ...
        switch (property) {
            ...
            case 'morph': {
                const pathArr = (obj as any).path;
                return Array.isArray(pathArr) && pathArr.length > 0
                    ? pathArr.map((cmd: any[]) => cmd.join(' ')).join(' ')
                    : '';
            }
            ...
            case 'strokeDashes': {
                const arr = (obj as any).strokeDashArray;
                return Array.isArray(arr) && arr.length > 0 && arr[0] > 0 ? arr[0] : 0;
            }
            default: return 0;
        }
    }, [fabricCanvas]);
    ...
    return (
        ...
        if (fabricCanvas) { try { fabricCanvas.renderAll(); } catch { /* canvas may be destroyed during remount */ } }
                    ...
                    <div className="text-xs text-slate-400 font-mono">
                        <span ref={timeLabelRef} className="text-indigo-400 font-bold">{currentTime.toFixed(2)}s</span> / {duration.toFixed(1)}s
                    </div>
                </div>
            ...
            <div className="flex flex-col w-full bg-slate-900/20 rounded border border-slate-900 overflow-y-auto min-h-0">
                ...
                <div className="flex w-full border-b border-slate-900 bg-slate-950/90 text-[10px] text-slate-500 font-mono h-6 items-center sticky top-0 z-10">
                    ...
                    <div className="flex-1 relative h-full overflow-hidden" style={{ zoom: timelineZoom / 100 }}>
                        ...
                        <div ref={rulerPlayheadRef} className="absolute top-0 bottom-0 w-0.5 bg-rose-400 z-40 pointer-events-none shadow-lg shadow-rose-500/50"
                ...
                {animatedLayerIds.length === 0 ? (
                    ...
                    <div ref={trackRef} className="flex flex-col divide-y divide-slate-900/60 relative"
                        ...
                        <div ref={trackPlayheadRef} className="absolute top-0 bottom-0 w-0.5 bg-rose-400 z-30 pointer-events-none shadow-lg shadow-rose-500/50"
                            style={{ left: `calc(${LABEL_WIDTH}px + (100% - ${LABEL_WIDTH}px) * ${playheadPercent / 100})` }} />
                        <div ref={dragHandleRef} className="absolute top-0 w-3 h-3 -ml-1.5 -mt-1 z-40 cursor-pointer"
                            style={{ left: `calc(${LABEL_WIDTH}px + (100% - ${LABEL_WIDTH}px) * ${playheadPercent / 100})` }} />

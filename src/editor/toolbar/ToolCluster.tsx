...
export default function ToolCluster({
    ...
    return (
        <div ref={ref} className={`relative inline-flex ${className || ''}`}>
            ...
            {isOpen && (
                <div className="absolute left-0 top-full mt-1 w-48 rounded-lg bg-slate-900 border border-slate-800 shadow-xl z-[999] p-1.5 flex flex-col gap-1">
                    {items.map((item) => (
                        <button
                            key={item.id}
                            onClick={() => { onSelect(item.id); setIsOpen(false); }}
                            className={`flex items-center gap-3 w-full text-left px-3 py-2 text-xs font-medium rounded-md transition-colors ${item.id === activeId

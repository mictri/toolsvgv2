...
import { applyPathString, interpolateMorphPath } from './morphHelper';
...
function applyDashToFabric(targetObj: fabric.Object, rawValue: any) {
    const numericDash = typeof rawValue === 'number' ? rawValue : Number(rawValue) || 0;
    targetObj.set('strokeDashArray', (numericDash > 0 ? [numericDash, numericDash] : null) as any);
}
...
function trackValueToGSAP(property: string, value: any): Record<string, any> {
    switch (property) {
        ...
        case 'strokeOffset': return { strokeDashOffset: value ?? 0 };
        case 'strokeDashes': return { dashValue: value ?? 0 };
        default: return {};
    }
}
...
export const compileTimeline = (
    ...
    const tl = gsap.timeline({
        ...
        yoyo,
        onUpdate: () => {
            if (!fabricCanvas || (fabricCanvas as any).isDisposed) return;
    ...
    for (const ao of animatedObjects) {
        ...
        for (const track of ao.tracks) {
            ...
            const isDashTrack = track.property === 'strokeDashes';
            const isMorphTrack = track.property === 'morph';
            ...
            if (isDashTrack) {
                const dashVal = Number(firstVal.value) || 0;
                const proxy = { dashValue: dashVal };
                tl.set(proxy, { dashValue: dashVal }, Math.max(0, firstVal.time));
                applyDashToFabric(targetObj, dashVal);
            } else if (isMorphTrack) {
                const pathD = typeof firstVal.value === 'string' ? firstVal.value : '';
                const proxy = { d: pathD };
                tl.set(proxy, { d: pathD }, Math.max(0, firstVal.time));
                if (targetObj && pathD) {
                    applyPathString(targetObj as fabric.Path, pathD);
                }
            } else if (isColorTrack) {
            ...
            for (let i = 0; i < sorted.length - 1; i++) {
                ...
                if (isDashTrack) {
                    const currDashVal = Number(curr.value) || 0;
                    const nextDashVal = Number(next.value) || 0;
                    const proxy = { dashValue: currDashVal };
                    tl.to(proxy, {
                        dashValue: nextDashVal,
                        duration: segDuration,
                        ease: next.easing === 'none' ? 'none' : next.easing,
                        onUpdate: () => {
                            applyDashToFabric(targetObj, proxy.dashValue);
                            targetObj.setCoords();
                        },
                    }, Math.max(0, curr.time));
                } else if (isMorphTrack) {
                    const startD = typeof curr.value === 'string' ? curr.value : '';
                    const endD = typeof next.value === 'string' ? next.value : '';
                    if (!startD || !endD) continue;
                    const proxy = { progress: 0 };
                    const easeType = next.easing === 'none' ? 'none' : next.easing;
                    tl.to(proxy, {
                        progress: 1,
                        duration: segDuration,
                        ease: 'none',
                        onUpdate: () => {
                            const d = interpolateMorphPath(startD, endD, proxy.progress, easeType);
                            if (d) {
                                applyPathString(targetObj as fabric.Path, d);
                                targetObj.setCoords();
                            }
                        },
                    }, Math.max(0, curr.time));
